module.exports = () => {
            ...
            for (let FolderName of Foldres) {
                ...
                for (let FileName of Files) {
                    ...
                    for (let i = 0; i < ARR.length - 1; i++) {
                        ...
                        let max = Number(lineArr[2]);
                        let close = Number(lineArr[4]);
    
                        let longSell = +(open * (1 + PERC)).toFixed(2);
    
                        if(max >= longSell) {
                            sum += ((longSell / open) - 1) * sum;
                            sum += (1 - (close / longSell)) * sum;
                            I_succ++;
                        }
                        else {
    ...
    let t = 'Vega_v1_short_all_dif.txt';
    let h = 'Best Perc;Profit;Succ;Loss;Ttl\n';
    let data = `${BestPerc.toFixed(5)};${((MaxSum - 1)*100).toFixed(2)}%;${Succ};${Loss};${Ttl}\n`
