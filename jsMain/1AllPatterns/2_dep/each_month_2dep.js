module.exports = (TF,requireScript,modeName,InrevalPerc,DeltPerc) => 
{
    ...
    for (let FolderName of Folders) {
        ...
        if(P1 < P2) c(P1 - DeltPerc,P2 + DeltPerc, FolderName);
        else c(P2 - DeltPerc,P1 + DeltPerc, FolderName);

        h += `${FolderName.split('.').join('-')};${((maxSum - 1)*100).toFixed(1)}%;${P1.toFixed(5)};${P2.toFixed(5)};${S};${L};${T}\n`;
